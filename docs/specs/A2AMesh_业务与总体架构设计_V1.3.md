...
→ 每次进度/终态先原子更新 Redis 快照并写 outbox，执行不等待 Relay
...
`CancelTask` 先记录内部 cancel-requested，再发控制消息到 owner instance。Runtime 子进程组实际退出，并确认不存在不可逆副作用或补偿成功后，Task 才进入 `TASK_STATE_CANCELED`。存在不可逆或 `UNKNOWN` 副作用时进入 `TASK_STATE_FAILED`，并在扩展元数据中标记 `reconciliation_required=true`；取消超时也失败并告警，不能伪造成功取消。
