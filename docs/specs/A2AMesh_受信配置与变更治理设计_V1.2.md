...
| `components` | componentType、稳定 componentPrincipal、nodeId、允许 instance NKey/selector、requiredForProfiles、schema/feature/version 范围、recovery verificationMethod/expectedDigest；READY 候选唯一映射 |
...
启用Artifact/Object Store的generation必须在`components[]`列出`artifact-adapter`稳定Principal/nodeId/NKey selector和`requiredForProfiles`。该NKey只获NATS §16.6的artifact create/finalize/delete（closed operation仅`REQUEST`）、hold create/renew/release、唯一`artifact.source.commit`、自身inbox及READY overlay；旧target-centric ref增删subject不得出现在rendered ACL。State对source.commit仍重验source owner AuthProof、source path tuple及跨全部touched Artifact的版本/内容，adapter transport身份不能自授跨目标能力，也不能提交artifact.delete `COMPLETE`。组件/READY缺失、ACL漏source.commit或多授旧ref/Task/Reconciliation/Recovery subject、跨两个Artifact正反例未通过均NACK。

同一generation还必须在`components[]`列出独立`artifact-hold-reaper`稳定Principal、nodeId、NKey selector、binary digest和`requiredForProfiles`。该NKey只获`a2a.v1.state.artifact.hold.expire` closed `SCAN|EXPIRE|REPLAY_CLAIM`、自身inbox和READY overlay；State另验持久due membership/score或terminal commit及candidate lease的owner Principal/instance、lease expiry、issuance ID、fence、token、hold digest和Artifact version；higher-fence replay必须先由State唯一claim writer持久化新candidate，裸整数拒绝。`artifact-hold-reaper`不得与`artifact-adapter`、`artifact-delete-worker`、Reconciliation、State或Runtime复用Principal/NKey，不得获得source/ref/delete mutation或Object Store物理删除凭据；组件/READY缺失、ACL漏expiry subject、多授其它State subject、过期lease/旧candidate或commit-before-reply exact replay正反例未通过均NACK。

同一generation还必须列出`artifact-delete-worker`稳定Principal、nodeId、NKey selector、binary digest和`requiredForProfiles`。该NKey只获`a2a.v1.state.artifact.delete`的closed `COMPLETE`、自身inbox和READY overlay，并且是唯一可解析Object Store provider物理删除凭据的运行组件；State必须按认证Principal+operation拒绝Adapter伪造COMPLETE或Delete Worker提交REQUEST。`artifact-adapter`、`artifact-hold-reaper`、`artifact-delete-worker`三者的Principal和NKey selector必须两两不同，任何共用、漏slot或ACL扩权均使generation NACK。
...
每个`components[]`运行组件条目必须显式含非空trimmed `nkeySelector,verificationMethod,expectedDigest`；每个deployment descriptor `fixedBaseSlots[]`条目必须显式含稳定`readyReporterPrincipal,probeNkeySelector,verificationMethod,expectedDigest`，其中expectedDigest都是64位小写SHA-256。全体fixed slot的`componentPrincipal/readyReporterPrincipal/probeNkeySelector`与全体`components[]`的`componentPrincipal/nkeySelector`进入同一全局身份空间：任意Principal值或NKey selector值不得被另一个slot、probe或运行组件复用。只比较完整stable slot tuple不足以证明身份隔离；发生复用时必须在RequiredSlotSet计算前REJECTED。

`RequiredSlotSetV1(profileName,bundle,deploymentDescriptor)`是所有profile/READY/Recovery合同唯一的稳定slot算法；`profileName`和每个`components[].requiredForProfiles[]`值都只接受大小写敏感的closed `CORE|INTEROP|EXTENDED`。算法先从signed deployment descriptor取得固定基础slot集合`CONFIG_CONTROLLER,STATE_SERVICE,GATEWAY,NATS_JETSTREAM,OBJECT_STORE,ARTIFACT_BROKER,AUDIT_SINK`，每个类型至少一个稳定slot；再取`components[]`中`requiredForProfiles`数组按UTF-8精确包含`profileName`的组件slot；最后并入signed `deliveryProfile.explicitlyRequiredOperationalSlots[]`，其中每项恰含`componentType,componentPrincipal,nodeId`并必须逐字节解析到`components[]`或deployment descriptor中的现有稳定slot，不能凭空创建身份。固定基础slot、profile组件slot和显式operational slot均只以`(componentType,componentPrincipal,nodeId)`作为身份；Core内嵌Merge Broker归属于`application-core`，不得作为额外operational slot。动态`instanceId`、process PID、NKey seed和READY requestId不得进入required slot集合。结果先去重，再按三字段UTF-8字节严格升序生成`deliveryProfile.requiredSlots[]`；Config Controller的签名前生成路径只从authoritative source字段计算该数组，不依赖尚不存在的projection。数组签入bundle后，validator必须要求`deliveryProfile.requiredSlots[]`存在，并从`components[]`与deployment descriptor独立重算后逐字节比较；缺失、caller删减、追加或通配符都拒绝。`recoveryPolicy.requiredComponents[]`同样是必填signed projection且必须恰等于同一`RequiredSlotSetV1`的带证明投影：每项恰含`componentType,componentPrincipal,nodeId,verificationMethod,expectedDigest`，按前三字段同序排列；fixed slot的后两字段必须逐字节等于descriptor条目，运行组件则必须逐字节等于对应`components[]`条目，不能只做格式检查或由recoveryPolicy另行自报。因此requiredSlots、requiredComponents、RestoreReceipt的componentVerificationDigests三者稳定slot集合必须完全相等，proof metadata也必须来自同一权威输入。任意profile少报/多报、重复/空slot、profile名称大小写/空格漂移、动态instanceId冒充稳定slot、deployment descriptor与bundle不一致、verificationMethod替换或expectedDigest格式/值漂移均REJECTED。

每个`RequiredSlotSetV1`成员都必须有可达的READY证明路径：`components[]`运行组件以自身允许NKey/AuthProof调用`config.ready`；固定基础slot必须使用descriptor显式绑定的`readyReporterPrincipal`与独立`probeNkeySelector` credential，并按同条目的`verificationMethod/expectedDigest`验证。State重验probe结果后仍以该stable slot的`componentType,componentPrincipal,nodeId`签发`ComponentReadyAttestationReceiptV1`。不得由Config Controller以自身Principal代报、由其他slot复用Principal/NKey/receipt、因组件无本地NKey而静默省略，或把一个probe receipt复制给多个slot。
