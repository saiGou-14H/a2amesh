...
同一generation还必须在`components[]`列出独立`artifact-hold-reaper`稳定Principal、nodeId、NKey selector、binary digest和`requiredForProfiles`。该NKey只获`a2a.v1.state.artifact.hold.expire` closed `SCAN|EXPIRE`、自身inbox和READY overlay；State另验持久due membership/score及candidate lease的owner Principal/instance、lease expiry、scan ID、fence、token、hold digest和Artifact version。`artifact-hold-reaper`不得与`artifact-adapter`、`artifact-delete-worker`、Reconciliation、State或Runtime复用Principal/NKey，不得获得source/ref/delete mutation或Object Store物理删除凭据；组件/READY缺失、ACL漏expiry subject、多授其它State subject、过期lease/旧candidate或commit-before-reply exact replay正反例未通过均NACK。
