...
```json
{
  ...
  "supportedInterfaces": [
    ...
    {
      "url": "nats+wss://mesh.example.com/a2a.v1.rpc/windows-a",
...
示例中的域名和 NATS URL 是占位符。生产 Card 由受信配置生成，不硬编码；Runtime 自报信息只能作为健康输入，不能直接扩展公开 Skill/Interface。

## 3.4 Card 生命周期

```text
Peer 配置/Runtime/Skill 变化
→ 生成 canonical Card
→ 官方 SDK 解析校验
→ 计算 content hash/ETag
→ State Service upsert_card(generation)
→ 发布 CardChanged 事件
→ Gateway 缓存失效
```
...
- 每个 `agentId` 同时只有一个由部署配置指定的 Card publisher；其他 instance 只更新自身 presence，不竞争 Card generation；
...
- `eventSequence` 只在 A2AMesh 增强路径用于排序/补发；
...
- Card builder 输入来自版本化受信配置；同一 agentId 的 publisher ownership、generation 和目标 delivery profile 必须通过启动校验。
...
10. 大 Artifact 不进入 Redis；hash/size/mediaType 可校验。
11. 日志和协议对象不包含 NKey seed、Token、环境变量或思维链。
12. v0.3 `message/send` fixture 不能被误当 v1 成功门禁。
13. **TEST-CARD-OWNER-001**：双 instance 同时启动时只有指定 publisher 可更新 Card，presence 仍分别可见。
14. **TEST-CARD-PROFILE-001**：CORE Card 不提前声明 gRPC/MCP/Push；通过 INTEROP 门禁后才增加 gRPC interface。
---
