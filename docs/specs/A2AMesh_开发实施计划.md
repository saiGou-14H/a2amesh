...
截至 2026-08-17 14:26（Asia/Shanghai），第一轮 remediation `6baef3d`、第三代 `436a562` 和第五代 `4bda50b` 的独立复审均为 `BLOCKED`。第四代 `fd0965d` 的限界同树复审未覆盖stream DTO P1；第六代 `5633dbe`、第七代 `0f27d14` 又分别被本地封存对抗探针和第五代完整复审的第二项P1取代，已进入第八代候选：
...
- 新增独立对抗回归覆盖mutable claim/credential、两类verifier map及属性、hostile hash、unknown derived A2A、retryable integer、两类generation bool、两类AuthContext混淆、全部stream DTO字符串/时间alias、三类语义伪造和server封存绕过；当前hardening专项 `31 passed`，C1-1/stream专项 `137 passed`，全量 `438 passed, 8 skipped`，8/8机器门禁通过；
- 本修复只针对C1-1独立复审发现，不实现 Redis State、Task 状态机、11 操作业务语义或真实 NATS/JetStream 故障验收；因此不满足 C1 整体退出门禁，也不代表完整 A2A v1 兼容或生产就绪；
- 第八代checkpoint、同树独立复审和提交后完整门禁完成前，状态保持 `candidate/pending-review`，不得写 `[verified]`。
