...
- 第五代补齐不同owner与returned iterator的双重关闭、missing/invalid `__aiter__`的固定映射；cleanup对Future cancel失败使用base fallback，对异步close使用独立future和50ms有界等待，父任务取消不被吞掉，次级异常不覆盖主异常；
...
- 新增独立硬编码11-operation registry matrix，及伪造protobuf、malformed awaitable/iterator/`__anext__`返回值、distinct owner、bounded cleanup、aiter建立、校验顺序、bool generation、错误控制字符、validator误判/误拒和NATS边界负例；当前第五代Core focused `60 passed`、NATS/response专项 `73 passed`、全量 `442 passed, 8 skipped`、8/8机器门禁与Ruff通过；
