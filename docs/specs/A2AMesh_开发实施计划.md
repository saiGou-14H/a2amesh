...
本次状态核验绑定代码基线 `d0ba51d2752feb754f6425e8f775361f3a764547`（`main`/`origin/main`，核验时工作树仅含本次 V1.6/V1.2 文档候选修改），维护者为 A2AMesh 项目维护者，最后核验时间 2026-08-14。当前代码仍是私有 NATS RPC 原型。源码/测试数量随提交变化，不在计划中手工维护；后续状态更新必须记录新的 commit SHA、dirty state、命令输出和证据 URI。
...
17. Artifact metadata、owner tombstone、artifact-centric typed refs/source forward index、create/renew/release hold、add/remove ref、retention lock及完整 finalize/delete/Reaper CAS。
...
5. 实现 DATA-RECOVERY-001 的canonical payload、非detached General JWS、Manifest/summary DAG/`indexRootDigest`、ArchiveTransition/Verification/Restore/Approval/Release receipts、checkpoint-source interface、delete journal、双人release和外部证据重建；实现archive写入→exact read-back→transition receipt→新summary/Manifest→独立递归验证→hot-range CAS compaction及`TEST-DR-MANIFEST-DAG-001`/`TEST-DR-COMPACTION-001`全部crash/replay门禁。本阶段接入Redis/Object/config/audit与确定性JetStream fixture，C3提供真实JetStream source。
...
- Artifact finalize/terminal/delete/ticket/hold/ref/Reaper 并发不产生悬空引用、计数泄漏或复活对象；
...
16. 实现真实JetStream checkpoint source并接入C2.5 Recovery Manifest summary DAG/indexRoot与archive transition；Redis/JetStream水位、summary range/count或archive receipt不一致时恢复门禁拒绝。
...
- `TEST-NATS-ACL-001`与`TEST-NATS-STREAM-SESSION-001`在真实broker通过：87/87 State subject有且仅有授权角色及READY overlay；Peer→本地Application Core→State与Dispatch Worker→Task Supervisor→command.get/lease/accept均可达且越权反例拒绝；单open reply及历史响应逐字节稳定；snapshot覆盖窗口不死锁；无inactive自动删除；跨Controller/Provisioner稳定接管；旧签名not-found/已消费ticket无法重放；final/expired清理无早终态、重复broker副作用或consumer泄漏；`TEST-IPC-REPLAY-001`证明Core journal重启边界不重复执行。
...
- CORE 必须通过 TEST-IDENTITY-001、TEST-TENANT-001、TEST-OUTBOX-001、TEST-EFFECT-001、TEST-AUTHZ-001、TEST-ADMISSION-001、TEST-ARTIFACT-001、TEST-CONFIG-ATOMIC-001、TEST-RECON-RESOLVE-001、TEST-RECON-IMMUTABLE-001、TEST-AUDIT-SINK-001、TEST-DR-MANIFEST-001、TEST-DR-001；MCP/OAuth 测试仅在 EXTENDED 强制；
