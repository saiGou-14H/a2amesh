...
每个 P1 告警必须链接 Runbook，至少包括：影响、快速判断、禁止操作、恢复步骤、数据一致性检查和升级联系人。重点 Runbook：NATS 不可用、Redis 不可写、Event Relay/outbox 堵塞、lease split-brain、Projector lag、effect UNKNOWN 对账、Windows orphan process、Push SSRF、OAuth AS/JWKS outage、AuthContext 签名异常、Credential/grant rotation、准入计数修复、备份恢复。
