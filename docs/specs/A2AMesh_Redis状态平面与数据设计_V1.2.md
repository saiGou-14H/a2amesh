...
1. 先验证 AuthContext 并调用 `resolve_principal`；把最终 Principal 和 aliasGeneration 固化进 Task。
2. dedupe 不存在：创建 dedupe、Task、状态/时间/Context/Principal/Agent 索引，返回 `CREATED`。
3. dedupe 存在且 hash 相同：返回原 taskId 与当前 Task，`DUPLICATE_SAME`。
4. hash 不同：返回 `DUPLICATE_CONFLICT`，不修改任何状态。
5. newTaskId 必须由 State Service 生成，调用方不可指定。
