...
| `recovery-verifier:<instanceId>` | `a2a.v1.state.recovery.verify`、`a2a.v1.state.recovery.restore` | `_INBOX.a2amesh.recovery-verifier.<instanceId>.>` | 无；唯一VerificationReceipt/RestoreReceipt writer，State必须匹配独立verifier signer和producer-different约束；不得调用seal/release/compact或Plan subject |
