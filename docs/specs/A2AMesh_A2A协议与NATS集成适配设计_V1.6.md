...
5. Controller崩溃由`reclaim_stream_session`取得更高fence，以State记录中的同configGeneration、consumerConfigJson/digest、consumerName、稳定controller delivery subject和持久watermark恢复；不得读取当前active generation重新生成配置。旧实例即使仍收到同一delivery，也因fence拒绝且不得ACK/NAK，新owner不需迁移consumer deliver_subject。Provisioner create/info/delete均按`streamSessionId+consumerConfigDigest`幂等。
6. final live frame必须依次完成caller ACK→State ACK permit→实际JS ACK→Provisioner INFO确认ack floor/无pending→State broker-ack confirm→CLOSING→delete→INFO-not-found→CLOSED。若已存initialFrame本身终态，则`initialFrame.final=true`，不等待不存在的live frame/JS ACK；Core成功发布并flush已存openedResponseJson后，必须先调用`a2a.v1.state.stream.flush`写入openedResponseFlushed确认，Controller读取该确认后才可CLOSING→delete/not-found→CLOSED。同streamOpenId重试逐字节返回同一openedResponseJson和currentState，即使Task后续version/审计/结果引用变化也不得重建。caller close可直接CLOSING并放弃pending；deadline/slow consumer走EXPIRING→delete→INFO-not-found→EXPIRED，不得提前写CLOSED/EXPIRED。
...
- **TEST-DISPATCH-001**：分别在 CLAIMED 后、SENT 后/NATS publish 前、publish 后/reply 前杀 Worker；再分别在Supervisor完成`task.command.get`后、取得provisional lease后、`accept_dispatch_and_start`提交前后杀Supervisor或并发cancel。accept提交前必须零Runtime进程、零effect attempt、零外部progress，provisional lease可被cancel/reclaim撤销；accept提交后仍必须完成observe-without-spawn→签名REGISTER_CONTAINMENT→State READ_CONTAINMENT exact read-back，任何一步前都保持零Runtime/零effect/零外部调用，只有完整链成功才允许启动exact command。过期 claim 必须以更高 attempt/token 接管，旧 token/reply 拒绝，客户端不重试时 Task 最终 ACCEPTED 或按 deadline 确定失败。
...
| `application-core:<agentId>:<instanceId>` | `a2a.v1.state.task.claim`、`a2a.v1.state.task.get`（closed `GET|LIST` variant）、`a2a.v1.state.task.cancel`、`a2a.v1.state.task.append`、`a2a.v1.state.push.config`（closed CRUD variant）、`a2a.v1.state.stream.flush`、`a2a.v1.state.agent.list`、`a2a.v1.state.agent.search`、`a2a.v1.state.principal.resolve`；为入站流式操作请求`a2a.v1.stream.open`；获授权的literal control | `_INBOX.a2amesh.core.<agentId>.<instanceId>.>` | 无；只接受同机Peer Binding/Gateway经受保护IPC转交的canonical envelope+verified credential observation，State仍重验Principal/capability/target；caller ACK/close/delivery仍由原caller Peer/Gateway身份处理 |
...
| `gateway:<instanceId>` | 路由展开的每个 `a2a.v1.rpc.<target>` literal；`a2a.v1.stream.open`、`a2a.v1.stream.ack`、`a2a.v1.stream.close`；`a2a.v1.state.task.claim`、`a2a.v1.state.task.get`（closed `GET|LIST` variant）、`a2a.v1.state.task.cancel`、`a2a.v1.state.task.append`、`a2a.v1.state.push.config`（closed CRUD variant）、`a2a.v1.state.card.get`、`a2a.v1.state.agent.list`、`a2a.v1.state.agent.search`、`a2a.v1.state.principal.resolve`；获授权的 literal `a2a.v1.control.<agentId>.<instanceId>` | `_INBOX.a2amesh.gateway.<instanceId>.>`；`_DELIVER.a2amesh.stream.gateway.<instanceId>.*` | 无 |
...
| `stream-session-controller:<instanceId>` | `a2a.v1.state.stream.open`、`a2a.v1.state.stream.broker-op.begin`、`a2a.v1.state.stream.broker-op.consume`、`a2a.v1.state.stream.activate`、`a2a.v1.state.stream.frame`、`a2a.v1.state.stream.ack`、`a2a.v1.state.stream.broker-ack`、`a2a.v1.state.stream.close`、`a2a.v1.state.stream.expire`、`a2a.v1.state.stream.cleanup`、`a2a.v1.state.stream.reclaim`；`a2a.v1.js.consumer.create`、`a2a.v1.js.consumer.info`、`a2a.v1.js.consumer.delete`；`_DELIVER.a2amesh.stream.*.*.*` | `a2a.v1.stream.open`、`a2a.v1.stream.ack`、`a2a.v1.stream.close`；`_DELIVER.a2amesh.controller.<meshId>.*`；`_INBOX.a2amesh.stream-controller.<instanceId>.>` | `allow_responses {max: 1, expires: 5s}`；只回复open/ack/close，不以reply发送帧；State校验session owner/fence/broker operation ticket，Controller无裸`$JS.API.*` |
...
| `artifact:<instanceId>` | `a2a.v1.state.artifact.create`、`a2a.v1.state.artifact.finalize`、`a2a.v1.state.artifact.delete`、`a2a.v1.state.artifact.hold.create`、`a2a.v1.state.artifact.hold.renew`、`a2a.v1.state.artifact.hold.release`、`a2a.v1.state.artifact.ref.add`、`a2a.v1.state.artifact.ref.remove` | `_INBOX.a2amesh.artifact.<instanceId>.>` | 无 |
...
| `recon:<instanceId>` | `a2a.v1.state.recon.open`、`a2a.v1.state.recon.claim`、`a2a.v1.state.recon.evidence`、`a2a.v1.state.recon.resolve`、`a2a.v1.state.recon.close`、`a2a.v1.state.recon.reopen`、`a2a.v1.state.effect.scan-stale` | `_INBOX.a2amesh.recon.<instanceId>.>` | 无；scanner只读stale-due并以独立operation幂等 |
| `audit:<instanceId>` | `a2a.v1.state.audit.claim`、`a2a.v1.state.audit.ack` | `_INBOX.a2amesh.audit.<instanceId>.>` | 无 |
| `recovery:<instanceId>` | `a2a.v1.state.recovery.seal`、`a2a.v1.state.recovery.verify`、`a2a.v1.state.recovery.restore`、`a2a.v1.state.recovery.release` | `_INBOX.a2amesh.recovery.<instanceId>.>` | 无 |
| signed `components[]` READY overlay | 同一组件NKey额外且仅额外获得`a2a.v1.state.config.ready` | 复用该组件既有私有inbox | State要求AuthProof signer、componentPrincipal/nodeId/instanceId与signed bundle一一匹配；request必须声明`readinessPlane=CANDIDATE`或`PRODUCTION_GATED`，后者还须当前rollout lease/fence、deployed ACL/stream/environment且trafficGate=CLOSED；不能替其他组件报告，非components身份无此overlay |
...
a2a.v1.state.artifact.hold.renew a2a.v1.state.artifact.hold.release
a2a.v1.state.artifact.ref.add  a2a.v1.state.artifact.ref.remove
...
Peer Binding与同机Application Core只允许下列本机IPC profile，禁止loopback TCP、继承stdio、临时文件、共享内存裸队列或Peer直接复用Core NKey。`agentKey=lowerBase32NoPad(SHA-256(UTF8(agentId)))[0:26]`；active signed config的`components[]`必须为本Agent的`peer-binding`与`application-core`分别固定稳定component Principal、NKey selector、`ipcProfileVersion=1`、host OS principal和binary digest，主机服务管理器实际身份/二进制任一不匹配即READY NACK。

| OS | endpoint与ACL | 双向本机身份校验 |
|---|---|---|
| Linux | `AF_UNIX/SOCK_STREAM`端点固定`/run/a2amesh/<agentKey>/peer-core.sock`；父目录由root预建、非符号链接、owner为Core UID、mode `0710`，socket owner为Core UID、group为仅含Peer/Core UID的专用GID、mode `0660`；每次启动先`lstat`并拒绝symlink/world-writable/错误owner | Core对accept socket读取`SO_PEERCRED`并要求UID/GID恰等于signed component映射的Peer OS principal；Peer连接后同样读取peer credentials并要求server UID恰等于Core UID。PID只用于审计，授权不依赖PID复用；root/主机管理员属于部署信任边界，不映射为应用Principal |
| Windows | Named Pipe固定`\\.\pipe\a2amesh\<agentKey>\peer-core`，以`PIPE_REJECT_REMOTE_CLIENTS`创建；SDDL仅授予Core service SID full control、Peer service SID read/write/connect，显式拒绝Anonymous/Network/Everyone，禁止继承宽DACL | Core在每次连接上`ImpersonateNamedPipeClient`并以`TokenUser`要求exact Peer service SID，随后立即`RevertToSelf`；Peer用`GetNamedPipeServerProcessId`取得server并校验其process token包含exact Core service SID。任一API不可用、远程client、错误SID或宽DACL均fail closed |

唯一request frame为`u32be(frameLen)||RFC8785_UTF8(LocalCoreRequestV1)`，`frameLen`仅计JSON bytes且必须在`1..16,777,216`。payload恰含`schemaVersion,ipcRequestId,ipcRequestDigest,agentId,peerBindingPrincipal,peerInstanceId,bindingVersion,receivedAt,expiresAt,bindingEnvelope,credentialObservation`；`ipcRequestId`为安全ULID，时间为UTC恰3位毫秒`Z`且窗口不超过5秒，`ipcRequestDigest=SHA-256(RFC8785(payload排除ipcRequestDigest))`。`credentialObservation`恰含`authMethod,credentialId,issuerHash,aliasGeneration,tokenDigest,verifiedAt`，nullable字段必须显式`null`；只传入口已验证观察值和Token HMAC/digest，绝不传Bearer/OAuth secret、私钥或预计算可信`principalId/AuthContext`。Core仍须按§6.5和State Principal/capability重新解析与授权，不能因OS身份跳过业务门禁。

唯一response frame使用同一length/framing规则；`LocalCoreResponseV1`恰含`schemaVersion,ipcRequestId,ipcRequestDigest,status,bindingResponse,error,responseDigest`，成功时`bindingResponse`非null且`error=null`，失败时相反，`responseDigest=SHA-256(RFC8785(payload排除responseDigest))`。Core必须在调用State/业务writer前将`LocalCoreReplayRecordV1`以durable journal append+fsync写入：字段恰为`schemaVersion,peerBindingPrincipalHash,ipcRequestId,ipcRequestDigest,businessOperationId,businessRequestDigest,state,createdAt,expiresAt,responseFrameBytes,responseDigest`，state为`IN_FLIGHT|COMPLETED`；journal record使用`u32be(frameLen)||RFC8785_UTF8(record)||u32be(crc32c)`，不完整/坏CRC尾记录只能截断到最后一个fsync边界，不能接受中间损坏。Core在有效期内按`peerBindingPrincipal+ipcRequestId`读取journal：同ID同digest且COMPLETED逐字节返回原response，同ID异digest拒绝；同ID为IN_FLIGHT时不得再次执行，只能用稳定`businessOperationId/businessRequestDigest`查询State既有业务幂等ledger，找到原result后补写COMPLETED并逐字节返回，找不到则返回固定`IPC_REPLAY_INDETERMINATE`并保持业务零重执行。业务result写入后、COMPLETED journal fsync前崩溃由该reconcile路径收口；Core重启后仍能在5秒窗口读取exact response，过期记录只能在fsync新索引/rename完成后清理。journal只缓存响应和digest，不保存secret/AuthContext/raw payload，权限固定为Core UID/专用GID或Windows Core service SID；Peer/Core不得把各自NKey、socket/pipe handle或OS token交给Runtime/Tool。

`TEST-NATS-ACL-001`的同机companion fixture必须在Linux和Windows各跑一组授权正例，并逐项拒绝错误UID/GID/SID、world-writable或继承宽ACL、symlink/stale endpoint、远程pipe、错误server身份、oversize/零长/截断frame、未知/额外字段、过期时间、同ipcRequestId异digest、raw secret/预计算AuthContext以及任意未授权本机进程；同时断言Peer进程没有Task mutation NKey，Core调用State后仍执行Principal/capability检查。`TEST-IPC-REPLAY-001`另注入journal append/fsync前、IN_FLIGHT fsync后、业务State CAS后、COMPLETED fsync前后、Core进程重启和坏尾恢复：同ID同digest在重启后逐字节返回原response，同ID异digest零写入，IN_FLIGHT不得重新执行业务而必须按businessOperationId查询既有ledger；ledger不存在时固定`IPC_REPLAY_INDETERMINATE`，过期journal安全清理，Linux/Windows权限边界和5秒窗口均验证。
