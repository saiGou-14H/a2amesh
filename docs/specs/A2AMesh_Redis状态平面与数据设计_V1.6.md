...
| `...:artifact:<artifactId>:holds:<holdId>` | HASH | reason,sourceCaseId/sourceTaskId,createdBy,expiresMs,status,digest,currentExpiryCandidateLeaseId | hold 释放/过期后保留审计；ACTIVE current incarnation若有未消费authority，该pointer必须指向最高fence entry，terminal时清空 |
| `...:artifact:<artifactId>:holds` | SET | active hold IDs | Reaper 原子读取 |
| `...:artifact:holds:due` | ZSET | member=`UTF8(artifactId)+0x00+UTF8(holdId)`，score=`expiresMs` | create/renew/release/expire/delete同Artifact CAS维护；只作候选索引 |
| `...:artifact:hold-expiry-candidate-fence` | STRING counter | 全局candidate fencing high-water；每次SCAN或REPLAY_CLAIM签发新candidate时原子递增 | 无物理TTL；严格单调，永不回退、按Artifact重置或复用；不得低于已持久candidate entry最大fence，预留或故障形成的gap永久burned且不得回填 |
| `...:artifact:hold-expiry-candidate:<candidateLeaseId>` | HASH | issuanceOperation=`SCAN`或`REPLAY_CLAIM`,scanOperationId,candidateLeaseId,candidateFencingToken,candidateToken,ownerPrincipalId,ownerInstanceId,issuedAtMs,leaseUntilMs,leaseUntilMs-issuedAtMs<=300000ms,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,candidateDigest,status,consumedByExpireOperationId,consumedAtMs | wire字段仍名`scanOperationId`：SCAN时查scan ledger，REPLAY_CLAIM时其值等于replayOperationId并查claim ledger；status仅`ACTIVE`或`CONSUMED`，消费CAS写后两字段并永久保留tombstone，逻辑lease过期不删除authority且禁止恢复ACTIVE/重新签发 |
| `...:artifact:hold-expiry-candidate-authority:<leaseOrToken>` | HASH | kind=`LEASE_ID`或`BEARER_TOKEN`,candidateLeaseId | lease ID与bearer token共享覆盖历史SCAN/REPLAY_CLAIM的全局create-if-absent keyspace；无物理TTL，ACTIVE/CONSUMED均阻止跨issuance operation及跨角色复用 |
| `...:artifact:hold-expiry-scan:<scanOperationId>` | HASH | operation=`SCAN`,requestDigest,candidatesJson,resultJson,resultDigest,createdMs | 与Artifact保留期同周期；同ID exact replay；只为普通ACTIVE/due scan，不能证明terminal replay authority |
| `...:artifact:hold-expiry-replay-claim:<replayOperationId>` | HASH | operation=`REPLAY_CLAIM`,expireOperationId,baseCommitDigest,requestJson,requestDigest,resultJson,resultDigest,candidateLeaseId,candidateDigest,candidateFencingToken,ownerPrincipalId,ownerInstanceId,issuedAtMs,leaseUntilMs,leaseUntilMs-issuedAtMs<=300000ms,createdMs | 与commit/Artifact/审计同周期且不可覆盖；保存exact request/result/nested candidate；同ID同bytes逐字节重放，异request零写入冲突；`baseCommitDigest`必须等于immutable commit digest |
| `...:artifact:hold-expiry-replay-current:<expireOperationId>` | HASH | replayOperationId,baseCommitDigest,candidateLeaseId,candidateDigest,candidateFencingToken,leaseUntilMs,revision | 无物理TTL；每个expire operation唯一current replay authority；其引用的candidate ledger entry必须满足`leaseUntilMs-issuedAtMs<=300000ms`。未过期时不同claim ID零写入且high-water不推进；逻辑过期后新claim CAS才可写严格更高fence/revision；一旦推进，所有低fence claim及原commit-fence authority均永久superseded |
| `...:artifact:hold-expiry:<expireOperationId>` | HASH | schemaVersion,expireOperationId,requestDigest,expectedHoldDigest,authorizedCandidateDigest,authorizedReaperPrincipalId,authorizedReaperFencingToken,previousArtifactVersion,remainingActiveHoldCount,activeRefCount,minimumDeleteAtMs,committedAtMs,resultJson,commitDigest | 与Artifact/审计同周期；immutable commit evidence；`authorizedCandidateDigest`必须等于唯一CONSUMED candidate canonical digest并由commitDigest覆盖。public writer只从权威snapshot内部查找；same-fence仅原candidate instance，higher-fence必须由上述current pointer、exact claim ledger和仍存活未消费candidate共同证明，裸整数/非current/低fence全部拒绝；replay还要求同一hold incarnation、terminal projection、commit↔tombstone↔audit↔outbox一一对应且当前version不低于已提交result，不从当前Artifact重建结果 |
...
- `renew_artifact_hold` 要求同 capability、expected Artifact/hold digest、ACTIVE hold；expiresMs 只能延长（legal hold 可为 null），同一幂等键相同请求返回现状。`release_artifact_hold`只把 hold 置 RELEASED/写 releasedMs，不删记录，并在同CAS从active set/retention lock/`artifact:holds:due`移除；`expire_artifact_hold`只接受Artifact Hold Reaper经`a2a.v1.state.artifact.hold.expire`提交的closed `SCAN|EXPIRE|REPLAY_CLAIM` wire。SCAN按1..100 limit读取持久due ZSET；纯合同`apply_artifact_hold_expiry_scan`在CAS模型中先校验ACTIVE/due/server time、Reaper认证上下文、owner/lease、完整due tuple和全局fence，再原子写exact result、candidate ledger及current projection；真实Redis Function/NATS handler仍未实现。candidate的issuedAtMs不得早于observedExpiresMs。新scan从lease/token共享的全局create-if-absent authority namespace及单调fence high-water签发绑定authenticated Principal/instance、完整due tuple、issued/lease-until和双32-byte token的candidate authority，并把ACTIVE current incarnation的`currentExpiryCandidateLeaseId`更新到该tuple最高fence entry。EXPIRE要求`serverNowMs>=observedExpiresMs=current expiresMs`且严格小于candidate leaseUntil，expected hold digest/Artifact version、due membership/score、scan/lease/fence/token及owner全等；public writer只从同一权威snapshot内部读取commit，不能接收caller提供的commit/result。在同一CAS写ACTIVE→EXPIRED、active set/retention lock（含minimumDeleteAtMs）/due removal、Artifact version+1、当前candidate投影清除、authority ledger `ACTIVE→CONSUMED` tombstone、immutable commit evidence及恰一个确定性audit/outbox；tombstone↔commit↔两类event任一缺失、重复或digest漂移时整个write set拒绝。commit-before-reply按persisted canonical result bytes重放，即使当前Artifact后来继续增版；同commit fence只接受原candidate instance。新instance/higher-fence必须先调用`apply_artifact_hold_expiry_replay_claim`，State在同一terminal snapshot内要求request/result的`baseCommitDigest`逐字节等于immutable `commitDigest`，并绑定原CONSUMED candidate的`authorizedCandidateDigest`、完整due tuple及认证Principal；若`hold-expiry-replay-current:<expireOperationId>`仍有未过期claim，不同claim ID必须零写入且candidate high-water不推进。仅在无current claim或其lease已逻辑过期时，Function才从全局high-water分配下一fence，并同CAS持久化exact claim request/result、新lease/token、未消费candidate及current pointer；同claim ID exact replay不重新分配，异request冲突。随后EXPIRE replay只接受该operation当前最高、未消费且lease存活的claim candidate，Principal/instance/fence逐字相等；current pointer一旦推进，全部低fence claim及原commit-fence authority都拒绝。普通SCAN仍只处理ACTIVE/due hold；原EXPIRE request加裸higher整数无论transport上下文均拒绝。跨resource、错误schema/boolean、ACTIVE current projection、缺CONSUMED tombstone、缺/过期/已消费claim candidate或retention evidence矛盾均零写入。renew/release/delete与expire任一先CAS成功后另一方零写入。
...
- 同commitId/scope/requestDigest/exact bytes逐字节返回`TypedSourceCommitResultV1`，异digest、乱序/重复/四字段ref、path/ref漂移、expected集合少报被移除目标/多报无关目标或Artifact版本漂移均零写入。Function commit-before-reply重试不得重复Artifact version/audit/outbox；Artifact Delete Worker的DELETE先赢则source不可见/原version不变，source commit先赢则DELETE读取retention ref后409/423；
- `request_artifact_delete`只接受Artifact Adapter的`REQUEST`，原子读取`artifact:<artifactId>:refs`、全部active ArtifactHold、最低保留时间和policy；有任一锁返回409/423，不改状态；无锁才把状态置为DELETING并入队，不能在对象仍存在时写DELETED。独立Artifact Delete Worker持有provider物理删除凭据；它与Artifact Adapter、Artifact Hold Reaper使用不同Principal/NKey，物理删除前以expected Artifact version再次读取refs/holds，确认目标object version/delete marker不可读后才以自身身份提交同一artifact.delete subject内的`COMPLETE`。State按认证Principal+operation授权；Adapter不能COMPLETE，Hold Reaper不能REQUEST/COMPLETE或接触Object Store。与source-commit/create-hold竞争时只有一个CAS成功，若锁先提交则删除拒绝，若DELETING先提交则后续source commit/hold拒绝且source保持不可见或必须重试到新Artifact。
...
- 五个claim writer都在同一CAS写case/counter/due、`claim-operation` exact result、audit/outbox；scanner lease/scan也各自写持久operation result。相同operationId/scope/requestDigest逐字节返回原result且不增加revision/token/audit，异digest、旧revision/fence/candidate tuple零写入。EXPIRE/ESCALATE的`Idempotency-Key`等于candidate的`dueOperationId`，但最终`claimOperationId`按人工对账文档的通用claim域分离公式派生且不得等于`dueOperationId`；scanner接管不得改变最终claim operation。`TEST-RECON-OPERATION-ID-001`固定fixture必须由State、NATS wire和实现共同复算。
...
- **TEST-ARTIFACT-HOLD-EXPIRY-001**（DATA-ARTIFACT-001）：真实Redis Function在一个hash slot内原子执行SCAN→EXPIRE，持久due/scan/candidate-authority/high-water/CONSUMED tombstone/commit/audit/outbox；提交前后与响应丢失注入不得重复版本、fence、audit或结果，重启后同operation逐字节重放。
- **TEST-ARTIFACT-HOLD-REPLAY-001**（DATA-ARTIFACT-001）：真实Redis重启前后验证replay-claim ledger与current pointer；未过期current claim阻止新ID且high-water不推进，过期后新ID只推进一次，higher claim后低fence及原commit authority全部拒绝。逐字节校验`baseCommitDigest`、`authorizedCandidateDigest`、request/result/nested candidate和`issuanceOperation`映射；缺claim ledger/current pointer、裸higher fence、非current/过期/已消费candidate、同ID异request及任一cross-record digest漂移都零写入。
