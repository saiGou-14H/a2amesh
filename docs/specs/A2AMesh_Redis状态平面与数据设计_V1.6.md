...
| `...:artifact:hold-expiry-candidate-fence` | STRING counter | 全局candidate fencing high-water；每次新SCAN签发候选时原子递增 | 无物理TTL；严格单调，永不回退、按Artifact重置或复用；不得低于已持久candidate entry最大fence，预留或故障形成的gap永久burned且不得回填 |
| `...:artifact:hold-expiry-candidate:<candidateLeaseId>` | HASH | scanOperationId,candidateLeaseId,candidateFencingToken,candidateToken,ownerPrincipalId,ownerInstanceId,issuedAtMs,leaseUntilMs,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,candidateDigest,status,consumedByExpireOperationId,consumedAtMs | status仅`ACTIVE`或`CONSUMED`；消费CAS写后两字段并永久保留tombstone，逻辑lease过期不删除authority且禁止恢复ACTIVE/重新签发 |
| `...:artifact:hold-expiry-candidate-authority:<leaseOrToken>` | HASH | kind=`LEASE_ID`或`BEARER_TOKEN`,candidateLeaseId | lease ID与bearer token共享同一全局create-if-absent keyspace；无物理TTL，ACTIVE/CONSUMED均阻止跨SCAN及跨角色复用 |
| `...:artifact:hold-expiry-scan:<scanOperationId>` | HASH | operation,requestDigest,candidatesJson,resultJson,resultDigest,createdMs | 与Artifact保留期同周期；同ID exact replay；closed operation仅SCAN |
| `...:artifact:hold-expiry:<expireOperationId>` | HASH | schemaVersion,expireOperationId,requestDigest,expectedHoldDigest,authorizedReaperPrincipalId,authorizedReaperFencingToken,previousArtifactVersion,remainingActiveHoldCount,activeRefCount,minimumDeleteAtMs,committedAtMs,resultJson,commitDigest | 与Artifact/审计同周期；immutable commit evidence；public writer只从权威snapshot内部查找；replay要求同一hold incarnation、对应CONSUMED tombstone、current terminal projection、commit↔audit↔outbox一一对应、当前version不低于已提交result且当前fence不旧于commit fence，不从当前Artifact重建结果 |
