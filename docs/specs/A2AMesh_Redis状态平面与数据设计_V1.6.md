...
准入状态固定 `QUEUED → SELECTED → RUNNING → RELEASED`；SELECTED 对应已占用但尚未执行的 `reserved` slot。硬不变量为全局及每 Principal 的 `reserved + running <= maxRunning`：QUEUED→SELECTED 原子 `queued--/reserved++`，SELECTED→RUNNING 原子 `reserved--/running++`，各失败/取消路径只按当前状态减一次对应计数。Task claim 只创建 QUEUED，dispatch intent 此时不可 due。
...
| Key | 类型 | 内容 | TTL/清理 |
|---|---|---|---|
| `...:dispatch:<taskId>` | HASH | dispatchId,taskId,targetAgentId,state,dispatchAttempt,requestHash,payloadRef,payloadDigest,configGeneration,policySnapshotHash,bindingSchemaVersion,admissionSlotToken,nextAttemptMs,claimOwner,claimToken,claimExpiresMs,claimedMs,sentMs,replyInboxDigest,dispatchDeadlineMs,lastError | Task 终态后 24 小时 |
...
| `...:task:recovery:operation:<recoveryOperationId>` | HASH | taskId,expiredFencingToken,fromAttempt,toAttempt,recoveryDispatchId,requestDigest,resultJson,resultDigest,committedMs | 与Task及dispatch历史同周期 |
| `...:task:recovery:scan:<targetAgentId>:<recoveryScanIdHash>` | HASH | requestDigest,resultRef,resultDigest,createdMs | 与Task恢复热窗口同周期 |

状态固定为 `BLOCKED_ADMISSION → PENDING → CLAIMED → SENT → ACCEPTED`，取消可从任一未 ACCEPTED 状态进入 `ABORTED`，失败进入 `DEAD`。每次 `PENDING→CLAIMED` 同时令 `dispatchAttempt+1`，从独立 `INCR dispatch-claim-fence` 取得永不复用的 `claimToken`；只有当前 attempt/token 可写 SENT/ACCEPTED。Worker 必须在网络 publish 前调用 `mark_dispatch_sent` 令 CLAIMED→SENT；因此“标记 SENT 后、实际 publish 前”崩溃仍由过期 reclaim 恢复。`CLAIMED/SENT && claimExpiresMs<=now` 只能由 `reclaim_expired_dispatch` 原子回到 PENDING，清 owner/inbox、按稳定 dispatchId 退避并使旧 token 永久失效；deadline/cancel 已满足时改走 DEAD/ABORTED 终态，不得复活。只有 `select_admission_for_dispatch` 可令 BLOCKED_ADMISSION→PENDING；只有 State 在确认无已接受 owner、无 UNKNOWN/APPLYING effect 且超过 dispatch deadline 时可写 DEAD并把 Task 置 FAILED。`claim_message` 对新 Task 原子创建 immutable dispatch command，本记录的 payloadRef/hash/generation 不得由 Worker 改写。

`accept_dispatch_and_start`把Task置WORKING并发放execution lease时，必须在同一CAS把taskId写入目标Agent的recovery due ZSET；`renew_lease`只在owner/fence匹配时同时更新lease和ZSET score，任何终态、确定cancel或安全释放都在同一CAS移除。ZSET只提供候选，State仍逐项重验Task/lease/effect/admission；Supervisor不得直接读取Redis或自行选择任意taskId。
...
| `...:plan:due` | ZSET | member=planId，score=leaseUntilMs；仅RUNNING非终态Plan | 终态/显式释放原子移除 |
...
| `...:recovery:archive-transition:<transitionId>` | HASH | recoveryPointId,sourceType,fromWatermark,toWatermark,preIndexRootDigest,archiveUri,archiveDigest,archiveRecordCount,rangeDigest,receiptUri,receiptDigest,state,createdAt | WORM receipt/manifest覆盖前不得删除hot range |
...
Manifest的`configGeneration`所指signed bundle必须含closed `recoveryPolicy.requiredComponents[]`；每项恰含`componentType,componentPrincipal,nodeId,verificationMethod,expectedDigest`，按`(componentType,componentPrincipal,nodeId)`各字段UTF-8字节严格升序且不得重复/为空。集合必须等于固定恢复基础组件`CONFIG_CONTROLLER,STATE_SERVICE,GATEWAY,NATS_JETSTREAM,OBJECT_STORE,ARTIFACT_BROKER,AUDIT_SINK`各至少一个稳定slot与该delivery profile全部`requiredForProfiles`组件slot的并集；额外/缺失slot或expectedDigest非64位小写SHA-256均使bundle/Manifest拒绝。Restore required set只能从Manifest绑定的exact config bundle重算，caller不得自报或删减。
...
`RecoveryCompactionRequestV1`字段恰为`schemaVersion,transitionId,recoveryPointId,sourceType,fromWatermark,toWatermark,expectedPreIndexRootDigest,expectedSourceWatermark,archiveUri,archiveDigest,archiveRecordCount,rangeDigest,requestDigest,idempotencyKey,authProof`；`RecoveryCompactionResultV1`字段恰为`schemaVersion,transitionId,recoveryPointId,sourceType,fromWatermark,toWatermark,archiveDigest,archiveReceiptDigest,newManifestDigest,newIndexRootDigest,state,resultDigest`。transition state只允许`PREPARING_ARCHIVE→ARCHIVED→INDEX_SEALED→VERIFIED→HOT_DELETED`，错误或范围/receipt不确定进入持久`FAILED_CLOSED`；同transitionId+requestDigest逐字节返回原result，异body/version零写入。

执行顺序固定为：

1. Recovery Compactor以source-specific lease/fence读取expected source watermark和已验证的pre-index root；只冻结`(fromWatermark,toWatermark]`，不能读到未来写入或跨越未归档缺口。
...
6. 只有`VERIFIED` receipt、new Manifest/summary read-back和expected pre-root/source watermark仍匹配时，Redis Function才CAS删除或标记`(fromWatermark,toWatermark]` hot records，同时写transition state/audit/outbox；delete journal记录连续范围。CAS前崩溃只重放archive/receipt/DAG，CAS后崩溃由archive+receipt+transition ledger幂等补齐，绝不把“删除请求已提交”当作已归档。
7. `HOT_DELETED`后的Redis索引只保留transition/manifest/summary root可重建字段；恢复路径必须从archive exact bytes和summary DAG重建source index。任何compactor旧fence、错误pre-root、重复/重叠范围、缺receipt、只存digest不存exact archive、对象listing替代URI GET均零写入。

上述顺序在archive写入后、archive读回后、receipt写入后、summary seal后、Verification receipt写入后、hot-delete CAS前后和response丢失点分别注入崩溃；每个点重试不得产生第二archive、第二receipt、重复delete journal、范围缺口或改变已签manifest。`Recovery Manifest`本身不可原地compaction；每次归档/摘要变化都生成新的recoveryPoint和`previousRecoveryPointId`链。

### 5.21 DATA-STREAM-SESSION-001：NATS 流会话

| Key | 类型 | 内容 | TTL/清理 |
|---|---|---|---|
| `...:stream-session:<streamSessionId>` | HASH | streamSessionId,streamOpenId,requestDigest,operation,taskId,callerPrincipalHash,callerScope,callerInstanceId,state,configGeneration,consumerConfigJson,consumerConfigDigest,brokerOperationLeaseMs,brokerApiApplyMaxMs,consumerName,filterSubject,controllerDeliverySubject,callerDeliverySubject,initialTaskVersion,initialFrameJson,initialFrameDigest,openedResponseJson,openedResponseDigest,openedResponseFlushedMs,flushObservationId,flushOperationId,flushResultJson,flushResultDigest,snapshotEventSeq,snapshotCoveredAckedStreamSeq,snapshotCoveredAckedEventSeq,snapshotCoveredAckedPayloadDigest,lastDeliveredStreamSeq,lastDeliveredEventSeq,lastDeliveredSequence,lastAckedStreamSeq,lastAckedEventSeq,lastAckedSequence,lastAckedPayloadDigest,lastBrokerAckedEventSeq,lastBrokerAckedStreamSeq,pendingStreamSeq,pendingPayloadDigest,finalStreamSeq,finalEventSeq,finalSequence,finalBrokerAckConfirmed,brokerOpEpoch,brokerOpInvalidatedThroughEpoch,maxUnquiescedBrokerApplyUntilMs,brokerOpQuiesceUntilMs,createConfirmedEpoch,cleanupEpochLowerBound,deleteConfirmedEpoch,closeReason,consumerDeletedAt,controllerInstanceId,controllerFence,controllerLeaseUntilMs,expiresAt,lastError | CLOSED/EXPIRED后24小时 |
...
| `...:stream-session:due` | ZSET | member=streamSessionId，score=controllerLeaseUntilMs/expiresAt | CLOSED/EXPIRED 后移除 |
...
- 首次执行的 Task 必须为 SUBMITTED、admission=SELECTED 且 dispatch claim/target 匹配；恢复执行只能走 `claim_recovery_attempt`。两者都要求 `cancelRequested=0`、未终态、未超过 hard deadline；
- lease 不存在或已过期；
- `INCR fence` 获取新 token；
- `SET lease PX ttl NX`；
- 更新 owner/attempt/version；
- 同一原子函数写 `TASK_OWNER_CHANGED` outbox；
- 返回 token 和新 version。首次执行获得的是 **provisional lease**：在 `accept_dispatch_and_start` 成功前不得启动 Runtime、创建 effect attempt 或写外部 progress；Cancel 可在同一 Task/dispatch CAS 中撤销该 lease/fence。
...
- 其他非终态：设置 `cancelRequested=1`、version+1，并写 `TASK_CANCEL_REQUESTED` outbox；
...
- `claim_dispatch` 只认领 PENDING/due、admission=SELECTED 且 slotToken 匹配的 intent；原子令 `dispatchAttempt+1`，从独立 fence counter 发放单调 claimToken，写 CLAIMED/owner/claimedMs/claimExpiresMs，并返回完整 immutable `DispatchTask`；
...
- `accept_dispatch_and_start` 只接受 SENT，校验 dispatchId/attempt/claimToken、Task provisional lease/fencing、target/config/payload digest、admission slot、cancel/deadline/effect；同一原子写 dispatch ACCEPTED、Task SUBMITTED→WORKING、admission SELECTED→RUNNING、reservation/running counter和 `TASK_WORKING` outbox。中途无可观察半状态；
...
`recover_plan_lease`是既有**RUNNING且非终态**Plan的唯一接管入口：除`state=RUNNING`外，还要求owner lease已过期，或受审计handoff已清owner但fence>0。`COMPLETED/FAILED/CANCELED`无论是否残留PENDING/READY Step、owner/lease是否过期、recoveryState为何值都永久拒绝takeover。函数是一个有界原子begin/takeover：取得更高fencing、写新owner/lease；若recoveryState=NONE，则令`RECONCILING,recoveryEpoch+1,recoveryRevision+1,recoveryCursorStepId=<stableOrder首Step>,recoveryStartedMs=now`；若已经RECONCILING，则保留epoch/cursor、仅令recoveryRevision+1并接管。任一begin/takeover都写AuditEnvelopeV1。`renew_plan_lease`只接受state=RUNNING；在RECONCILING中只允许当前owner/fence续租，不解除gate。
...
`a2a.v1.state.task.recover`只允许目标Agent的Task Supervisor调用。每个Supervisor实例以新`recoveryScanId`周期调用`claim_recovery_attempt(targetAgentId,recoveryScanId,expectedConfigGeneration)`；同一网络调用重试必须复用scan ID。State按`(leaseUntilMs,taskId)`从该Agent recovery due ZSET中确定性选择首个`score<=now`候选，caller不能提供或替换taskId；无候选时也持久返回`NO_DUE`，相同scan ID+requestDigest逐字节返回原结果、同key异body冲突，下一轮轮询必须使用新scan ID。

选中候选后，State仅用于lease丢失后的非终态Task；caller不因持有该literal自动成为owner。函数原子校验targetAgentId、retryPolicy/maxAttempts/nextRetryMs、hard deadline、cancel、旧lease已失效、admission slot、workspace私有worktree、全部effect汇总。`recoveryOperationId=SHA-256("a2amesh-task-recovery-v1\0" || taskId || "\0" || expiredFencingToken || "\0" || fromAttempt)`；同一过期lease tuple无论由哪个scanner发现都命中同一operation记录并返回原`resultJson/resultDigest`，绝不再次递增attempt。存在UNKNOWN/APPLYING/APPLIED未安全处理时拒绝并创建/复用reconciliation case；允许时在一个CAS中写operation结果、移除due候选、递增Task attempt，创建唯一新的recovery dispatchId/intent（复用canonical command与policy snapshot），进入QUEUED admission。State提交后response丢失不影响该intent继续经DRR投递；原scanner重试得到原结果，新scanner不会创建第二intent。旧ACCEPTED dispatch保留为历史但不再投递；不得直接对旧ACCEPTED intent acquire新lease；新intent仍须经DRR、Dispatch Worker和`task.command.get→acquire_lease(provisional)→dispatch.accept`路径。
...
State mutation 在同一原子函数调用内部 helper `append_audit_event`，按 sourceSeq 分配 eventId/digest 并写 DATA-AUDIT-001；非 State 来源必须先取得 fsync WAL/Audit Ingress receipt。`claim_audit` 发放短 claim lease；`ack_audit_receipt` 只接受 WORM sink 返回的 globalSeq/segment/digest/receipt，原子写 DELIVERED 与 source ack watermark。dead 事件阻塞该 source 的 checkpoint，只有带运维 capability 的原 event 重投可恢复；不得生成新 eventId 掩盖缺口。

### 6.22 `seal_recovery_manifest` / `verify_recovery_manifest` / `mark_restored` / `release_recovery_point`

四个操作严格实现 DATA-RECOVERY-001 状态机和 expected state/digest CAS，并以 `recoveryPointId+receiptType+payloadDigest` 幂等：

- `seal_recovery_manifest` 只接受完整连续 sources、RFC 8785 payload 与满足两个不同 kid 阈值的 exact JWS General JSON；外部 WORM 返回 immutableUri/manifestJwsDigest/receipt 后才写 SEALED；
- `verify_recovery_manifest`只允许与producer不同的verifier Principal，实际读取全部URI/restore probe并先持久化`VerificationReceiptV1`；State在单一CAS前从SEALED manifest重算七个source exact顺序/URI/sourceDigest/verificationMethod，逐项要求observedDigest相等、result=VERIFIED、verificationDigest公式相等，并重算restoreProbeDigest和receipt result=VERIFIED。只有全部谓词、receipt URI/exact envelope digest/manifest digest/signer/time同时成立才写VERIFIED/latest-verified；空数组、缺失/重复/额外/错位、FAILED/UNKNOWN或任一digest错均零写入并进入REJECTED审计；
- `mark_restored`要求上述valid VerificationReceipt，并从Manifest configGeneration的exact signed bundle重算非空requiredComponents集合；State逐项要求Restore array与该集合exact相等、排序/字段/expected=observed/result=PASSED/verificationDigest全部成立，再从完整数组重算restoredVerificationDigest并要求receipt result=RESTORED。只有WORM RestoreReceipt exact URI/envelope digest/signer/time也有效时，一个CAS保存URI/digest/restoredVerificationDigest并写RESTORED；caller自报required set、空/少/多/重复component或失败结果均零写入；
- `release_recovery_point` 要求两个不同 operatorPrincipal 且不同 kid 的独立 Approval JWS、相同 manifest/restoredVerification digest 与 APPROVE decision；先写 ReleaseReceipt，再原子写两份 approval 与 release receipt 的全部 URI/digest、RELEASED、latest-released 和 AuditEnvelopeV1。相同 approvals 重试返回原结果，任一异 digest/同 Principal/同 kid 冲突。

任何步骤都不批量改写 source 事实，也不能用 Redis 内索引或 timestamp 代替外部签名正文；Redis 全损重建必须逐份重验 manifest、receipts 和 approvals。

### 6.23 `open_stream_session` / `begin_stream_broker_operation` / `claim_stream_broker_operation` / `complete_stream_broker_operation` / `consume_stream_broker_operation` / `activate_stream_session` / `claim_stream_frame` / `ack_stream_session` / `confirm_stream_broker_ack` / `close_stream_session` / `expire_stream_session` / `finish_stream_session_cleanup` / `reclaim_stream_session`

`open_stream_session`原子校验AuthProof claim、caller对Task的归属/operation、streamOpenId幂等、deadline和active config，创建DATA-STREAM-SESSION-001 OPENING记录、确定性subjects、controller lease/fence，并持久写入configGeneration、consumerConfigJson/digest及signed brokerOperationLeaseMs/brokerApiApplyMaxMs；同digest重试返回原记录，异digest拒绝。`requestDigest=SHA-256(RFC8785({schemaVersion,streamOpenId,operation,taskId,callerScope,callerInstanceId,configGeneration,expiresAt,consumerConfigDigest}))`，排除requestDigest/AuthProof/transport requestId；时间按UTC三位毫秒Z canonical string，nullable显式null。consumerConfigJson字段恰为`schemaVersion,configGeneration,streamName,consumerName,filterSubject,controllerDeliverySubject,deliverPolicy,ackPolicy,replayPolicy,maxAckPending,ackWaitMs`；`deliverGroup`和`inactiveThresholdMs`都不得出现，broker语义固定为无queue group且不按inactive自动删除。任何Controller/Provisioner不得按当前active generation重新推导旧session。
...
`confirm_opened_response_flushed`是Application Core唯一的flush事实writer，request恰含`schemaVersion,flushOperationId,streamSessionId,controllerFence,openedResponseDigest,corePrincipal,flushedAt,authProof`。State只接受当前Core NKey、OPENING/ACTIVE session、已存openedResponseDigest完全相等且有效fence；`StreamFlushResultV1`字段恰为`schemaVersion,flushOperationId,streamSessionId,openedResponseDigest,acceptedAt,resultDigest`并把exact bytes/digest存入session。同`flushOperationId`同digest逐字节返回原结果，异digest冲突；一个CAS写`openedResponseFlushedMs,flushObservationId,flushOperationId,flushResultJson,flushResultDigest`及audit/outbox。`close_stream_session`从ACTIVE/终态snapshot关闭前必须看到该字段，Controller不得凭本地socket flush猜测；flush前/后丢响应和旧Core/fence均零写入。`a2a.v1.state.stream.flush`是唯一外部入口。
...
- **TEST-DR-COMPACTION-001 / DATA-RECOVERY-001**：覆盖archive写入/read-back、ArchiveTransitionReceipt、new summary/Manifest、独立Verification、hot-range CAS删除和response loss各前后崩溃点；同transitionId/body逐字节重放且不重复archive/receipt/delete journal，异digest/旧fence/错误pre-root/重叠watermark零写入。receipt/archive/summary未全部WORM可读前不得删除hot数据；Redis全损后仅凭Manifest、summary DAG、archive exact bytes和receipts恢复source index，缺任一保持FAILED_CLOSED。
---
