...
- `seal_recovery_manifest`只接受signed `recovery-orchestrator` NKey与manifest signer kid，并要求完整连续 sources、RFC 8785 payload与满足两个不同kid阈值的exact JWS General JSON；外部WORM返回immutableUri/manifestJwsDigest/receipt后才写SEALED；
