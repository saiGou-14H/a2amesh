...
`ArtifactHoldExpiryResultV1`字段恰为`schemaVersion,expireOperationId,artifactId,holdId,requestDigest,resultCode,holdStatus,artifactVersion,retentionLocked,auditEventType,outboxEventType,resultDigest`；成功固定`resultCode=EXPIRED,holdStatus=EXPIRED,auditEventType=ArtifactHoldExpired,outboxEventType=ArtifactHoldExpired`，`retentionLocked`必须是真正JSON boolean，`resultDigest`为排除自身后的RFC8785 SHA-256。SCAN request、candidate、SCAN result、EXPIRE request/result和commit每一类都必须从bytes执行严格ingress：拒绝非法UTF-8、NaN/Infinity、duplicate key、非object root、非RFC8785 exact bytes、缺/额外字段、错误JSON类型与未知enum，并递归验证nested candidate。跨实现固定字节见`tests/fixtures/state_contracts/artifact_hold_expiry_v1.json`，fixture必须完整包含全部wire canonical bytes以及commit evidence，不能只锁定部分字段。
...
`ArtifactHoldExpiryCommitV1`字段恰为`schemaVersion,expireOperationId,requestDigest,expectedHoldDigest,authorizedReaperPrincipalId,authorizedReaperFencingToken,previousArtifactVersion,remainingActiveHoldCount,activeRefCount,minimumDeleteAtMs,committedAtMs,resultJson,commitDigest`；`resultJson`是上述完整result的RFC8785 exact bytes，commitDigest使用独立`a2amesh-artifact-hold-expire-commit-v1` domain并绑定全部字段和exact result bytes。`artifact:hold-expiry:<expireOperationId>`保存该不可变commit evidence。public expiry writer只能在权威原子snapshot内按`expireOperationId`查找commit，接口不得接收caller提供的`stored_commit`、result或“已提交”布尔值。结果提交后响应丢失时，State先验证operation ID/request digest、transport注入的`componentType=artifact-hold-reaper`、literal subject、非空认证Reaper Principal/instance、当前fence不低于commit fence、result schema/严格类型、resource tuple、当前hold digest和expiry仍为同一incarnation、当前Artifact version不低于result version、当前hold projection已经体现`EXPIRED`且不在active set/due并无ACTIVE candidate投影、对应authority ledger条目已是绑定同一operation/time的`CONSUMED` tombstone、commit fence与被消费candidate fence逐字相等、commit time位于issued/lease窗口且不早于observed expiry、previous→result version和retention evidence，以及commit/audit/outbox的一一对应，再逐字节返回原result且snapshot bytes保持不变；同commit fence只允许原candidate owner instance重放；更高认证Reaper fence允许新instance使用**原EXPIRE request**只读重放，但不得通过普通SCAN在terminal hold上签发或注入新candidate，且request必须仍逐项绑定已CONSUMED tombstone；单独传入更大整数仍须经过transport认证上下文。后续更高Artifact version仍可重放，旧fence、不同hold digest或早于commit的version拒绝。**不得用当前可变Artifact状态重建result**。异digest、跨artifact/hold结果、错误schema/非boolean、retention evidence矛盾、旧version/digest、过期或旧candidate、未到期、RELEASED/已缺due索引或DELETING/DELETED目标均零写入。renew/release/delete与EXPIRE共享Artifact version CAS，先提交者线性化，另一路返回冲突并保持原状态。
...
10. 有限期ArtifactHold必须通过Artifact Hold Reaper持久due/closed SCAN|EXPIRE唯一writer；`expireOperationId`绑定artifact/hold/digest/version/due tuple，candidate lease/owner/expiry/fence仅作当前授权；ACTIVE→EXPIRED、due/retention（含minimumDeleteAt）/version、immutable commit evidence/audit/outbox同CAS，响应丢失从canonical result bytes逐字节重放。物理删除仅由使用不同Principal/NKey的Artifact Delete Worker完成，Hold Reaper不得持有provider删除凭据。
