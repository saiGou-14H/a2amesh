...
删除请求先通过 State CAS 原子读取 Task/case/evidence 反向索引、全部 active ArtifactHold、minimumDeleteAt 和 policy；任一锁存在返回 409/423且不改变状态。无锁才把状态改为 `DELETING` 并写 outbox。Artifact Adapter 只允许提交 `REQUEST`；随后由独立 signed component **Artifact Delete Worker**（`artifact-delete-worker`稳定Principal/NKey）取得删除工作，它与Artifact Adapter、Artifact Hold Reaper均不得复用Principal/NKey，并且是唯一可取得Object Store provider物理删除凭据的运行组件。Delete Worker删除前再次校验Artifact version、hold/ref、policy generation和无迟到finalize，按provider policy处理历史version/delete marker；确认目标版本不可读后，才以自身认证身份提交同一`a2a.v1.state.artifact.delete` subject内的`COMPLETE`，State按caller Principal与operation继续做fail-closed授权并写`DELETED`。重复删除返回当前状态；存储超时保持`DELETING`并重试，不把“请求已接受”写成“已删除”。finalize与delete以Artifact version CAS线性化，迟到copy只能成为受控orphan，不能恢复AVAILABLE。Artifact Hold Reaper不得执行物理对象删除，也不得获得provider删除凭据。
...
### 5.6 ArtifactHold expiry（内部State控制面）

**Artifact Hold Reaper** 是有限期hold的唯一expiry actor；业务Credential、Artifact Adapter、Artifact Delete Worker和Reconciliation Service均不得调用该接口。内部NATS subject固定为`a2a.v1.state.artifact.hold.expire`，只接受closed `SCAN|EXPIRE|REPLAY_CLAIM` union。三种business payload都封装在已签名NATS Envelope/AuthProof内；`authProof`属于transport envelope而不是下列business request字段。State入口必须同时要求transport注入的`componentType=artifact-hold-reaper`、literal subject、认证Principal/instance和当前Reaper authority context，不能信任payload自报actor。

`ArtifactHoldExpiryScanRequestV1`字段恰为`schemaVersion,operation,scanOperationId,maxCandidates,idempotencyKey,requestDigest`；`operation=SCAN`，`maxCandidates`为1..100，`scanOperationId/idempotencyKey`均为lowerhex SHA-256且逐字节相等，`requestDigest`为排除自身后的RFC8785 SHA-256。纯状态合同入口`apply_artifact_hold_expiry_scan`不接收caller提供的candidate/lease/token/fence；它在一个CAS模型内校验ACTIVE hold、due membership/score、State server time、配置的Reaper Principal/instance、完整due tuple和全局fence，由内部State allocator按持久high-water恰加1分配fence、以CSPRNG生成互异token，并产生绑定request/candidate/previous-high-water/digest的私有allocation proof。ledger缺proof、proof与result重绑或previous-high-water不等于持久前驱均零写入；SCAN result、candidate ledger、allocation proof、SCAN authority seal及current candidate projection随后作为同一immutable snapshot验证。该proof只证明当前Python进程内的State-side allocation合同；真实Redis Function/Lua必须实现等价原子分配与持久provenance，当前仍未实现。SCAN从持久`artifact:holds:due`中选择`score<=State.serverNowMs`的候选，并为每项原子签发短期、单次candidate lease；V1固定`leaseUntilMs-issuedAtMs<=300000ms`（5分钟），State拒绝超出该上限的candidate；candidate的`issuedAtMs`不得早于`observedExpiresMs`；不再存在未实现的`ACQUIRE|RENEW`长租约操作。每个`ArtifactHoldExpiryCandidateV1`字段恰为`schemaVersion,scanOperationId,candidateLeaseId,candidateFencingToken,candidateToken,ownerPrincipalId,ownerInstanceId,issuedAtMs,leaseUntilMs,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,candidateDigest`；V1为兼容既有EXPIRE wire沿用字段名`scanOperationId`，在REPLAY_CLAIM result的nested candidate中其值必须逐字节等于`replayOperationId`，此时语义是candidate issuance operation ID而不是普通due SCAN；两个token都必须为canonical 32-byte Base64URL且互异，candidateDigest为排除自身后的RFC8785 SHA-256。`candidateLeaseId`与`candidateToken`共享一个覆盖全部历史SCAN的全局持久authority namespace，二者必须互异、全局create-if-absent且ACTIVE/CONSUMED均永久占位；`candidateFencingToken`必须严格大于持久high-water，不能按SCAN或Artifact重新计数。相同SCAN ID仅允许逐字节返回原签发结果，不得重新分配任一lease/token/fence；不同SCAN即使due tuple相同也不得复用历史authority。`ArtifactHoldExpiryScanResultV1`字段恰为`schemaVersion,scanOperationId,requestDigest,candidates,resultDigest`，候选按`(artifactId,holdId)` UTF-8严格排序且不得重复；result除自校验digest外还必须通过`validate_for(originating ScanRequest)`逐字节复验scanOperationId、requestDigest和maxCandidates。SCAN exact result保存在`...:artifact:hold-expiry-scan:<scanOperationId>`，candidate ledger保存在`...:artifact:hold-expiry-candidate:<candidateLeaseId>`，lease/token统一反向占位保存在`...:artifact:hold-expiry-candidate-authority:<leaseOrToken>`，全局fence high-water保存在`...:artifact:hold-expiry-candidate-fence`；snapshot validator必须要求high-water不低于全部已持久candidate entries中的最大fence（无entry时可为0或更高的已burned值）；counter已前进但未形成candidate的gap必须永久视为burned，后续签发仍严格大于当前high-water，不得回填；对当前ACTIVE hold incarnation/due tuple，hold上的current candidate投影必须逐字节等于所有未CONSUMED ledger entries中的最高fence candidate，缺失或仍指向旧fence均拒绝，terminal hold只保留ledger历史而不得恢复current投影。

`ArtifactHoldExpiryRequestV1`的EXPIRE字段恰为`schemaVersion,operation,expireOperationId,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,scanOperationId,candidateLeaseId,candidateFencingToken,candidateToken,idempotencyKey,requestDigest`；`operation=EXPIRE`，`idempotencyKey`必须逐字节等于`expireOperationId`。稳定due tuple的exact bytes为`UTF8("a2amesh-artifact-hold-expire-v1")+0x00+UTF8(artifactId)+0x00+UTF8(holdId)+0x00+UTF8(expectedHoldDigest)+0x00+ASCII(expectedArtifactVersion)+0x00+ASCII(observedExpiresMs)`，`expireOperationId=lowerhex(SHA-256(exact bytes))`。`requestDigest=lowerhex(SHA-256(RFC8785({schemaVersion,operation,expireOperationId,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,idempotencyKey})))`；临时`scanOperationId/candidateLeaseId/candidateFencingToken/candidateToken`以及transport AuthProof不进入business digest，使合法重新SCAN/接管可复用同一稳定operation，但State仍逐项校验当前candidate ledger。

`ArtifactHoldExpiryResultV1`字段恰为`schemaVersion,expireOperationId,artifactId,holdId,requestDigest,resultCode,holdStatus,artifactVersion,retentionLocked,auditEventType,outboxEventType,resultDigest`；成功固定`resultCode=EXPIRED,holdStatus=EXPIRED,auditEventType=ArtifactHoldExpired,outboxEventType=ArtifactHoldExpired`，`retentionLocked`必须是真正JSON boolean，`resultDigest`为排除自身后的RFC8785 SHA-256。SCAN request、candidate、SCAN result、EXPIRE request/result、REPLAY_CLAIM request/result和commit每一类都必须从bytes执行严格ingress：拒绝非法UTF-8、NaN/Infinity、duplicate key、非object root、非RFC8785 exact bytes、缺/额外字段、错误JSON类型与未知enum，并递归验证nested candidate。跨实现固定字节见`tests/fixtures/state_contracts/artifact_hold_expiry_v1.json`，fixture必须完整包含全部wire canonical bytes以及commit/replay-claim evidence，不能只锁定部分字段。

State Function只接收包含业务状态、全部SCAN结果、candidate authority ledger/high-water、immutable commits、audit与outbox记录的权威原子snapshot，并在一个CAS内验证：Artifact/hold ID、hold当前为ACTIVE、hold digest和Artifact version等于expected、`observedExpiresMs`仍等于当前expiresMs且`serverNowMs>=observedExpiresMs`、hold仍在active set和`artifact:holds:due`且score未变、有限期expiresMs非null；candidate ledger的scan ID、lease ID、owner Principal/instance、issued/lease-until、fence、token和完整due tuple均与认证上下文/request/current state完全匹配，并要求`issuedAtMs<=serverNowMs<leaseUntilMs`以及transport注入的当前fence逐字节等于candidate fence。因此旧owner在lease自然过期或当前fence推进后即零写入。成功CAS唯一允许`ACTIVE→EXPIRED`、Artifact version恰加1、精确移除当前active hold、移除due、清除hold上的当前candidate投影并按剩余hold/ref及`minimumDeleteAtMs`重算retention lock；同一个CAS必须把对应authority ledger条目从`ACTIVE→CONSUMED`并写`consumedByExpireOperationId/consumedAtMs`，永久保留lease/token/fence tombstone且禁止`CONSUMED→ACTIVE`。它还原子写一个immutable commit、恰一个确定性AuditEnvelopeV1记录和恰一个确定性outbox记录；candidate tombstone↔commit以及commit↔audit↔outbox必须分别一一对应，缺失、重复、跨operation、sink错位或digest漂移均使整个snapshot无效。Artifact lifecycle、hold digest/expiresMs、active ref count、minimumDeleteAtMs及其他active hold必须保持不变；Reaper不执行物理对象删除。

`ArtifactHoldExpiryCommitV1`字段恰为`schemaVersion,expireOperationId,requestDigest,expectedHoldDigest,authorizedCandidateDigest,authorizedReaperPrincipalId,authorizedReaperFencingToken,previousArtifactVersion,remainingActiveHoldCount,activeRefCount,minimumDeleteAtMs,committedAtMs,resultJson,commitDigest`；`resultJson`是上述完整result的RFC8785 exact bytes，commitDigest使用独立`a2amesh-artifact-hold-expire-commit-v1` domain并绑定全部字段和exact result bytes。`artifact:hold-expiry:<expireOperationId>`保存该不可变commit evidence。public expiry writer只能在权威原子snapshot内按`expireOperationId`查找commit，接口不得接收caller提供的`stored_commit`、result或“已提交”布尔值。结果提交后响应丢失时，State先验证operation ID/request digest、transport注入的`componentType=artifact-hold-reaper`、literal subject、非空认证Reaper Principal/instance、result schema/严格类型、resource tuple、当前hold digest和expiry仍为同一incarnation、当前Artifact version不低于result version、当前hold projection已经体现`EXPIRED`且不在active set/due并无ACTIVE due-candidate投影、对应authority ledger条目已是绑定同一operation/time的`CONSUMED` tombstone、commit fence与被消费candidate fence逐字相等、`authorizedCandidateDigest`与被消费candidate canonical digest逐字相等、commit time位于issued/lease窗口且不早于observed expiry、previous→result version和retention evidence，以及commit/audit/outbox的一一对应，再逐字节返回原result且snapshot bytes保持不变。同commit fence只允许原candidate owner instance exact replay。新instance或更高fence必须先调用唯一public writer `apply_artifact_hold_expiry_replay_claim`：`ArtifactHoldExpiryReplayClaimRequestV1`字段恰为`schemaVersion,operation,replayOperationId,expireOperationId,baseCommitDigest,artifactId,holdId,expectedHoldDigest,expectedArtifactVersion,observedExpiresMs,candidateLeaseId,candidateToken,idempotencyKey,requestDigest`，`operation=REPLAY_CLAIM`、`idempotencyKey`逐字节等于`replayOperationId`，且`replayOperationId=lowerhex(SHA-256(UTF8("a2amesh-artifact-hold-replay-claim-v1")+0x00+ASCII(expireOperationId)+0x00+ASCII(candidateLeaseId)))`；`requestDigest`为排除自身后的上述完整request RFC8785 SHA-256，`ArtifactHoldExpiryReplayClaimResultV1`字段恰为`schemaVersion,replayOperationId,expireOperationId,baseCommitDigest,requestDigest,resultCode,candidate,resultDigest`且成功固定`resultCode=CLAIMED`，`resultDigest`为排除自身后的完整result RFC8785 SHA-256。State只在已持久commit及同一terminal hold上校验完整due tuple、commit/result、`baseCommitDigest==commitDigest`、认证Principal/component/subject、唯一lease/token且该`expireOperationId`不存在未过期current claim authority后，才从持久high-water分配`+1` fence，并把claim request/result、新candidate及current-authority pointer（逐字段绑定candidate/result authority projection与revision）同一snapshot持久化；同一`replayOperationId`重试（包括其candidate lease已过期时）只逐字节返回已持久claim result，不重新分配authority，异request固定冲突。只要current claim lease未过期，任何不同claim ID都必须零写入且high-water不推进；lease过期后新claim ID才可取得严格更高fence，一旦持久化，所有低fence claim及原commit-fence authority均永久`superseded`。随后EXPIRE replay必须携带该未消费、request-bound candidate，认证Principal/instance/fence逐字相等且`issuedAtMs<=serverNowMs<leaseUntilMs`以及`leaseUntilMs-issuedAtMs<=300000ms`；过期后只能用新claim ID重新签发。普通due SCAN只允许ACTIVE/due hold，永远不能在terminal hold伪造replay authority；携带原EXPIRE request并仅把认证fence整数调高，无论是否存在transport上下文都拒绝。后续更高Artifact version仍可重放，旧fence、不同hold digest或早于commit的version拒绝。**不得用当前可变Artifact状态重建result**。异digest、跨artifact/hold结果、错误schema/非boolean、retention evidence矛盾、旧version/digest、过期或未持久claim、未到期、RELEASED/已缺due索引或DELETING/DELETED目标均零写入。renew/release/delete与EXPIRE共享Artifact version CAS，先提交者线性化，另一路返回冲突并保持原状态。

当前`src/a2amesh/state_contracts/artifact_hold.py`中的`ArtifactHoldExpiryCASState`与专项测试只证明上述严格wire、全局candidate ledger、CONSUMED tombstone、内部commit lookup和原子write-set的**确定性纯状态合同**；它不是Redis Function/Lua实现，也不证明Redis重启持久化、真实NATS handler/AuthProof、secure ACL、Object Store或多进程故障注入。C2/C2.5/C3对应里程碑在这些真实集成门禁通过前必须继续标记为未实现/未验收。
...
- **TEST-ARTIFACT-HOLD-REF-001**：并发create/renew/release/expire hold及TASK/CASE/EVIDENCE source commit与Artifact Hold Reaper/Artifact Delete Worker。正例用一个source-centric commit同时引用两个Artifact，下一sourceVersion以完整集合保留一个/移除一个，再以空refs移除全部；expectedArtifactVersions每次恰覆盖old∪new且每个touched Artifact version只+1。在canonical source/ref/content digest校验、CAS前后、State commit-before-reply、DELETE先/commit先各点杀进程。断言source visibility、五字段typed双向索引、retention lock、Artifact version、source-commit exact result和audit/outbox同CAS，任意时刻不存在VISIBLE source+missing ref；重复幂等逐字节重放且不重复计数。逐项拒绝旧四字段ref、path/ref tuple漂移、重复/乱序、missing/extra expected target、错误contentSha256、target-centric路径、一个target capability越权修改其他Artifact、旧version/digest及DELETING目标；ref移除只能由新sourceVersion完整commit。
- **TEST-ARTIFACT-HOLD-EXPIRY-001**：Artifact Hold Reaper经持久`artifact:holds:due`执行closed SCAN→EXPIRE，校验candidate lease ID/owner/instance/issued/lease-until/fence/token及完整due tuple；覆盖有限期hold到期、legal hold、minimumDeleteAt、renew/release/delete/expire四方CAS竞态、旧digest/version/candidate、过期lease、合法重新SCAN、State提交前后和commit-before-reply响应丢失。正例必须同CAS得到`ACTIVE→EXPIRED`、due/active set/retention lock/Artifact version、immutable commit evidence、AuditEnvelope/outbox；同ID同digest从canonical result bytes逐字节重放，即使当前Artifact后来继续增版也不重复版本/审计；跨resource、错误schema/非boolean、retention evidence矛盾、异digest和未到期请求均零写入，物理删除不由EXPIRE执行。
- **TEST-ARTIFACT-HOLD-REPLAY-001**：固定fixture逐字节复算`REPLAY_CLAIM` request/result/nested candidate/commit的RFC8785 bytes与digest，断言`baseCommitDigest==commitDigest`且commit的`authorizedCandidateDigest`逐字节等于原CONSUMED candidate digest。覆盖同claim ID exact replay、同ID异request冲突、未过期current claim阻止不同ID且high-water不推进、lease过期后新ID恰取更高fence、higher claim后全部低fence及原commit-fence authority拒绝、裸higher整数/未持久candidate/错误Principal或instance/过期或已消费candidate拒绝；逐字段改写owner instance、lease ID、token、issuance operation、issued/lease时间并重算candidate/scan/claim digest但保持commit不变时，snapshot与public replay均fail closed。nested candidate的`scanOperationId`在该variant必须逐字节等于`replayOperationId`，且真实Redis重启后request/result/current-authority pointer仍可exact replay。
...
10. 有限期ArtifactHold必须通过Artifact Hold Reaper持久due/closed SCAN|EXPIRE|REPLAY_CLAIM唯一writer；`expireOperationId`绑定artifact/hold/digest/version/due tuple，candidate lease/owner/expiry/fence仅作当前授权；higher-fence terminal replay必须先取得并持久化commit-bound、未消费且lease有效的新candidate，裸整数拒绝；ACTIVE→EXPIRED、due/retention（含minimumDeleteAt）/version、immutable commit evidence/audit/outbox同CAS，响应丢失从canonical result bytes逐字节重放。物理删除仅由使用不同Principal/NKey的Artifact Delete Worker完成，Hold Reaper不得持有provider删除凭据。验收绑定`TEST-ARTIFACT-HOLD-REPLAY-001`。
