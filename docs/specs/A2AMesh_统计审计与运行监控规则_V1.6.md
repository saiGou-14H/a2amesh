...
审计可靠投递以 `DATA-AUDIT-001` 为权威：State mutation 在同一 Redis Function 写 durable audit outbox；认证/OAuth 拒绝、Runtime/Tool、审计读取等无 State mutation 的来源，必须先写 fsync 本地 WAL 或 Durable Audit Ingress 并取得 receipt。配置、对账、恢复、副作用启动等特权操作在 durable audit 不可持久化时 fail closed。Audit Relay 用确定性 `eventId/sourceSeq` 投递到独立 append-only sink，获得持久 WORM receipt 后才 ack；普通日志和 Redis Stream 只是诊断/热缓冲。
