...
|---|---|
| `caseId` | 服务端生成，不透明且稳定 |
| `taskId/effectId/attempt` | 唯一关联一个 UNKNOWN effect attempt |
| `targetAgentId/provider/operation` | 从 ledger 复制的稳定脱敏维度 |
| `risk` | effect 创建时固化的风险等级 |
| `state` | `OPEN/CLAIMED/ESCALATED/RESOLVED/CLOSED` |
| `resolution` | 仅 RESOLVED/CLOSED 有值：`APPLIED/FAILED/COMPENSATED` |
...
同一个 `effectId + attempt` 最多一个 active case。重复 UNKNOWN 回调返回原 case，不创建重复告警或多个操作员工作单。

## 3.2 状态机

```text
UNKNOWN effect ── atomic create ──▶ OPEN ── claim ──▶ CLAIMED ── resolve ──▶ RESOLVED ── verify/archive ──▶ CLOSED
                                  │                    │
                                  └─ SLA/escalate ─────┴──────────▶ ESCALATED ── claim/resolve ──▶ RESOLVED
...
1. claim lease 到期不会删除 claim 记录；写 `ClaimExpired` 审计后，case 回到 `OPEN`，已升级 case 保持 `ESCALATED`。
2. `ESCALATED` 是优先级/时限状态，仍可被具备权限的操作员认领和解决。
3. `RESOLVED` 表示 ledger 已完成原子裁决；`CLOSED` 表示二次核验/归档完成，不改变 resolution。
4. reopen 必须提供新证据或指出原证据失效，增加 revision 并保留旧 resolution；不能删除旧审计。
5. reopen 原子把 `reconciliationRequired` 重新置为 true，但不把已裁决 ledger 倒回 `UNKNOWN`。若新证据支持原裁决，可用同一 resolution 再确认并追加审计；若证明原裁决错误，必须创建受控纠错/补偿 effect attempt，reopened case 引用新 effect 的结果，原 ledger 和旧 resolution 保持不可变。
...
证据写入 append-only。纠错通过新增 `EvidenceSuperseded` 关系，不能覆盖原记录。resolution 必须引用足以支持结果的 evidence ID；自由文本 comment 只能辅助说明。
...
- 释放 claim 只清除 active owner，不删除操作历史；case 回到 `OPEN` 或保持 `ESCALATED`。
...
1. 校验 case state/revision、active claim、fencing token、capability 和 evidence 引用。
2. 初次 resolution 校验 ledger 仍是相同 effect attempt 的 `UNKNOWN`，且 provider reference/idempotency hash 一致；reopened case 则校验原 resolution 不变，并要求“同结果确认”或新的纠错/补偿 effect 证据。
...
4. 写 case `RESOLVED`、resolution、revision 和 append-only audit。
...
列表支持稳定 cursor、`state/priority/provider/age` 过滤和确定排序，不允许按自由文本 provider body 搜索。mutating 请求必须携带 `Idempotency-Key`、`expectedRevision`；claim 之后的写操作还需 `claimFencingToken`。重复同请求返回原结果，同 key 不同 body、旧 revision 或旧 fencing token 返回 409。
...
- `read`、`claim`、`evidence.write`、`resolve`、`reopen` 分开授予；不存在通用 admin 通配。
...
| 15 分钟未 claim | `ESCALATED`，通知升级联系人 |
...
审计事件至少包含 case/effect/task ID、action、operator Principal hash、claim fencing token、expected/actual revision、evidence ID/digest、resolution、reason code、request/trace ID、时间和 config generation。metric label 不包含 caseId、taskId、Principal、provider reference 或自由文本。
...
- **TEST-RECON-EVIDENCE-001**：只有允许类型、完整 digest/来源/时间的证据可用于 resolution，自由文本不能单独裁决。
