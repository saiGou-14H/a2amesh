...
| Public Gateway | 标准 Agent Card、JSON-RPC/SSE/gRPC、版本与 A2A Bearer 入口 |
| Identity Resolver | 把 NKey/Bearer/OAuth 凭据映射为 Canonical Principal，输出可信 AuthContext |
| OAuth Authorization Server | 外部部署依赖，为 MCP machine client 签发 audience-bound Token；A2AMesh 不自行签发 |
...
可缩放源图：[SVG](../assets/A2AMesh_V1.2_Architecture.svg) · [自包含 HTML](../assets/A2AMesh_V1.2_Architecture.html)
...
- 维护 Principal/Credential/Alias Registry；`callerPrincipal` 只来自已验证 AuthContext。
...
- State Service；
- Public Gateway；
- Identity Resolver/Credential Registry；
- MCP Bridge；
- 外部 OAuth 2.1 Authorization Server 的部署或连接配置（参考域名 `auth.<baseDomain>`）；
- 可选 Linux Peer；
...
- **TEST-MCP-IDEMP-001**：同 Principal/target/messageId/payload 并发与超时重试只产生一个 Task；冲突 payload 拒绝。
- **TEST-OAUTH-001**：issuer discovery、audience、scope、JWKS rotation、过期、AS outage 和 fail-closed 通过。
