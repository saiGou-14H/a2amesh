# A2AMesh Agent Card 与协议对象规范 V1.0
...
```text
GET https://<gateway-domain>/.well-known/agent-card.json
```

部署可以使用每 Agent 子域名，也可由 Registry/直接配置返回 Card。无论发现方式如何，Card 必须是官方 `AgentCard` 对象。
...
| `securitySchemes` | 公网部署启用时声明 Bearer/OAuth2/mTLS；私有测试可为空 |
| `securityRequirements` | 与 Gateway 实际校验一致 |
...
示例中的域名和 NATS URL 是占位符。生产 Card 由配置生成，不硬编码。
...
```text
callerIdentity + targetAgentId + message.messageId
```
...
8. 大 Artifact 不进入 Redis；hash/size/mediaType 可校验。
9. 日志和协议对象不包含 NKey seed、Token、环境变量或思维链。
10. v0.3 `message/send` fixture 不能被误当 v1 成功门禁。
