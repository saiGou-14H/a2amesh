...
> 异步 TaskSupervisor + JetStream 单一事件源 + Redis 权威快照 + SSE 在线订阅 + A2A Push 离线通知 + GetTask 断线校准 + Observer 规则过滤。

```text
                              ┌─▶ State Projector ─▶ Redis ─▶ GetTask
TaskSupervisor                │
 ├ Runtime events             ├─▶ JetStream ─▶ SSE Gateway
 ├ process watchdog           │
 ├ task heartbeat             ├─▶ Push Dispatcher ─▶ Webhook
 ├ lease renewal              │
 └ cancel monitor             └─▶ Observer Rules ─▶ Observer Agent
```
...
所有周期可配置并加 jitter。heartbeat 事件可以不进入外部 Push，但必须更新 JetStream/Redis 水位和指标。
...
1. Core `claim_message` 生成/复用 Task。
2. 选择 target Agent，写 SUBMITTED。
3. NATS dispatch；Peer 领取 lease。
4. transition WORKING，phase=queued/runtime_running。
5. 启动 Runtime，收集事件和 heartbeat。
6. 产生 Artifact/状态事件。
7. finalizing，校验 lease 和退出码。
8. 写 terminal Task 和 Artifact 元数据。
9. 释放 lease、关闭 stream、清理 Push 配置策略。
...
Linux：新 process group + TERM grace + KILL。Windows：CREATE_NEW_PROCESS_GROUP/Job Object + grace + terminate tree。
...
4. 查询本地恢复记录/外部副作用状态；
5. retry-safe 任务新 attempt 接管；
6. unsafe 任务 FAILED 或等待人工；
...
Supervisor 可继续执行并发布 JetStream，但不能安全续 lease/终态写时应停止新副作用、进入受控等待；Core 停止新提交。恢复后 Projector 和 State transition 协调，无法确认的任务人工处理。
...
1. Runtime 60 秒无 stdout 仍每 5 秒 task heartbeat。
2. stdout 无换行不影响 heartbeat、deadline 和 cancel。
3. SSE keepalive 不被前端误判为任务进度。
4. SSE 断线后 GetTask/Subscribe 恢复，Task 不重复执行。
5. 多订阅者顺序一致，慢订阅者不拖住任务。
6. Push 失败不阻塞 Runtime，重复投递可去重。
7. lease 过期后旧 owner 不能写终态。
8. unsafe task 崩溃不自动重跑。
9. Observer 不处理普通 heartbeat，不响应自身事件。
10. Progress 可由官方 SDK 解析且不泄露思维链/secret。
