...
1. 同 messageId 同 payload 并发 100 次只生成一个 Task。
2. 同 messageId 不同 payload 全部冲突且不执行。
3. 两实例竞争 lease 只有一个成功，旧 token 不能晚写。
4. terminal 状态不能被 late working/cancel 覆盖。
5. upsert_card 旧 generation 被拒绝，heartbeat 不改 ETag。
6. ListTasks 游标在相同 updatedMs 下无重复/遗漏。
7. Projector 重放同 eventSeq 不重复改变版本。
8. Redis 重启后 Task/Card/dedupe/lease 语义符合预期。
9. Redis 端口从公网和 Windows 不可达。
10. 内存满时写失败并告警，不发生静默逐出。
