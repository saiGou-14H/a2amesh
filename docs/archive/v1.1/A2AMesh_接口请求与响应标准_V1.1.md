...
| `Authorization` | 生产固定 `Bearer <deployment-token>`，与 Card `meshBearer` 一致；仅隔离测试可关闭 |
...
metadata key 使用 lowercase：`a2a-version: 1.0`、`a2a-extensions`、`authorization: Bearer ...`、`...gRPC deadline/cancel 传给 Core，但传输取消不自动等同 `CancelTask`。反向代理按 TLS SNI/`:authority` 路由 agentId，并允许 HTTP/2 与 server streaming。
