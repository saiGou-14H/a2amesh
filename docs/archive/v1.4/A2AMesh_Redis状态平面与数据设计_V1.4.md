...
| `...:card:<agentId>:meta` | HASH | version,etag,cardGeneration,configGeneration,fencingToken,updatedMs,instanceId | 无短 TTL |
...
| Key | 类型 | 内容 | TTL/清理 |
|---|---|---|---|
| `...:artifact:<artifactId>` | HASH | taskId,ownerPrincipalHash,objectKey,contentSha256,sizeBytes,mediaType,status,configGeneration,version,timestamps | 按 Artifact policy |
...
| `...:config:active` | HASH | generation,bundleId,contentSha256,activatedMs | 无短 TTL |
| `...:config:component:<generation>:<componentType>:<instanceId>` | HASH | READY/NACK,reasonCode,observedMs,expiresMs | 短 TTL |
| `...:card-publisher:<agentId>` | HASH | instanceId,configGeneration,fencingToken,expiresMs | lease TTL |
| `...:config:audit` | STREAM | generation,action,result,operatorHash,reasonCode,timestamp | 热窗口后不可变归档 |

active pointer 只能由 Config Controller CAS 更新。bundle 不含 secret 明文；完整 validate/stage/activate/rollback/revoke 合同以《受信配置与变更治理设计》为准。

## 5.15 DATA-RECON-001：人工对账 case

| Key | 类型 | 内容 | TTL/清理 |
|---|---|---|---|
| `...:recon:case:<caseId>` | HASH | taskId,effectId,attempt,state,resolution,priority,revision,claimOwnerHash,claimExpiresMs,claimFencingToken,timestamps | 至少 365 天/归档 |
| `...:recon:effect:<effectId>:<attempt>` | STRING | 唯一 caseId | 与 case 同周期 |
| `...:recon:cases:<state>` | ZSET | member=caseId, score=updatedMs | 状态迁移原子维护 |
| `...:recon:evidence:<evidenceId>` | HASH | caseId,type,source,result,payloadSha256,artifactId,collectorHash,timestamps | append-only |
| `...:recon:case:<caseId>:evidence` | ZSET | member=evidenceId, score=collectedMs | 与 case 同周期 |
| `...:recon:audit` | STREAM | caseId,revision,action,result,operatorHash,evidenceDigest,timestamp | 热窗口后不可变归档 |
...
## 6.7 `begin_effect` / `complete_effect` / `resolve_reconciliation_case`
...
## 6.11 `create_upload` / `finalize_artifact` / `request_artifact_delete`

- `create_upload` 原子校验 Task owner/capability、大小/MIME/配额和 active policy generation，写上传会话；
- `finalize_artifact` 只接受 Artifact Broker 已验证的 object key、size、SHA-256 和 media type，原子写 AVAILABLE metadata、Task Artifact、version/eventSequence 和 outbox；
- completion 的 idempotency key + uploadId 重入返回原 Artifact，不重复事件；
- `request_artifact_delete` 把状态置为 DELETING 并入队，不能在对象仍存在时写 DELETED。

## 6.12 `stage_config` / `activate_config`

- `stage_config` 只接受已验证签名/hash/schema 的单调 generation，写不可变 bundle 和 STAGED audit；
- `activate_config` 校验 previousGeneration、必选组件 READY/TTL 和无 NACK，以 CAS 原子替换 active pointer、运行时索引和 outbox；
...
## 6.13 `claim_reconciliation_case` / `append_reconciliation_evidence`

- claim 原子校验 case revision，发放单调 fencing token 和默认 10 分钟 lease；
- evidence append-only，写稳定类型、digest、来源和可选受控 artifactId；
- 旧 revision、旧 fencing、重复 idempotency key 不同 payload 均冲突；
- 详细 resolve/reopen 规则见《人工对账与运维操作设计》。
...
redis_reconciliation_case_count{state,priority}
