...
重复 Cancel 幂等。终态 Task 返回当前状态或 `TaskNotCancelableError`（按官方 SDK 行为固定测试）。HTTP 请求超时不代表取消失败，调用方随后 GetTask。`reconciliation_required` 是 A2AMesh 可选扩展元数据，忽略扩展的客户端仍能看到标准 `TASK_STATE_FAILED`，不得把远端效果未知包装为成功取消。
...
| Event Relay publish/consumer | taskId + eventSequence |
...
| Reconciliation claim/evidence/resolution/reopen | operator Principal + caseId + `Idempotency-Key` + revision/fencing + canonical body hash |
